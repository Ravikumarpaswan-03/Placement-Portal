...
async function deleteSelfAccount() {
  if (!confirm("Are you sure you want to permanently delete your mentor account? This action cannot be undone and will delete all your records.")) {
    return;
  }
  const token = localStorage.getItem("token");
  try {
    ...
    if (response.ok) {
      alert("Your mentor account has been successfully deleted.");
      localStorage.clear();
      window.location.href = "../index.html";
    } else {
      const data = await response.json();
